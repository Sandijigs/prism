...
contract WorldIDGate is Ownable {
    ...
    /// @notice Cross-chain verification records per user.
    ...
    /// @notice Total number of uniquely verified users.
    ...
    /// @notice Emitted when a user is verified via cross-chain relay.
    event CrossChainVerified(address indexed user, uint64 sourceChainSelector, address sourceAddress);
    ...
    modifier onlyCREWorkflow() {
        if (msg.sender != creWorkflow) revert NotAuthorized();
        _;
    }
    ...
    // ══════════════════════════════════════════════════════════════════════
    //  Cross-Chain Verification (CRE Workflow)
    // ══════════════════════════════════════════════════════════════════════

    /// @notice Verify a user via cross-chain relay from a CRE workflow.
    /// @dev    Only callable by the authorised CRE workflow address. Records
    ///         the source chain and address for auditability.
    /// @param user                The address to mark as verified on this chain.
    /// @param sourceChainSelector Chainlink CCIP chain selector of the source chain.
    /// @param sourceAddress       The user's address on the source chain.
    function verifyCrossChain(
        address user,
        uint64 sourceChainSelector,
        address sourceAddress
    ) external onlyCREWorkflow {
        if (user == address(0)) revert ZeroAddress();

        crossChainVerifications[user] = CrossChainVerification({
        ...
        _setVerified(user);
        emit CrossChainVerified(user, sourceChainSelector, sourceAddress);
    }
